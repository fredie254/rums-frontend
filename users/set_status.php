...
if (!hash_equals($_SESSION['csrf_token'] ?? '', $csrf)) {
    set_flash('error', 'Invalid request.'); redirect(BASE_URL . '/users/index.php');
}
if (!$id || $id == $_SESSION['user_id']) {
    set_flash('error', 'Cannot modify your own status.'); redirect(BASE_URL . '/users/index.php');
}
if (!in_array($status, ['active', 'inactive', 'suspended'], true)) {
    set_flash('error', 'Invalid status.'); redirect(BASE_URL . '/users/index.php');
}
